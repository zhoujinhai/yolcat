...
## make dataset
### label data
Use [labelme](https://github.com/wkentaro/labelme) to label data. 
If an image have multiple objects of the same kind, the label need set same kind, for example: one dog / two person, 
the label are `dog`, `person`, `person`. The will generate a json file for image.
...
```
the `json_dir` is all json file which generated by `labelme` for all image.
the `save_dir` is save path that you want to save coco format data
the `label.txt` is your data's label, for example, you have `dog` and `person` classes, the label.txt context is follow:
```text
__ignore__
_background_
dog
...
```
the `image.json` file is generated by `labelme` after labeled.
